...
variable "data_disk_size_gb" {
  description = "The size of the data disk in GB attached to this instance.  {{UIMeta group=3 order=10 options=10,50,100 }}"
  type        = number
  default     = 10
}

variable "data_disk_type" {
  description = "Disk types for notebook instances data disk. {{UIMeta group=3 order=9 }}"
  type        = string
  default     = "PD_SSD"
}
...
variable "enable_gpu_driver" {
  description = "Install GPU driver on the instance. {{UIMeta group=3 order=11 }}"
  type        = bool
  default     = false
}
...
variable "gpu_accelerator_type" {
  description = "Type of GPU you would like to spin up. {{UIMeta group=3 order=12 }}"
  type        = string
  default     = ""
}

variable "gpu_accelerator_core_count" {
  description = "Number of of GPU core count. {{UIMeta group=3 order=13 }}"
  type        = number
  default     = 0
}

variable "image_family" {
  description = "Image of the AI notebook. {{UIMeta group=3 order=15 }}"
  type        = string
  default     = "pytorch-latest-cpu-debian-11-py310"
}

variable "image_project" {
  description = "Google Cloud project where the image is hosted. {{UIMeta group=3 order=14 }}"
  type        = string
